...
if __name__ == '__main__':
    root="/home/okugawa/HDNNP/Si-200917/"
    ...
    updsmalldata=root+"small/result/RMSETCdata.csv"
    plotfile=root+"small/result/RMSETC-LC7LC3LC1small.png"
    colors=["orange","red","purple","pink","b","green","cyan","brown","darkgray"]
    lbls=["small-0.5","small-0.99","small2-0.5","small2-0.99","LC7m-0.5","LC7m-0.99","LC3m-0.99","LC1-0.99","T5L7-0.99"]
    ...
    with open(updsmalldata,'r') as f6:
        updsm0599= csv.reader(f6)
        L8, L9 = 0,0
        for row in updsm0599:
            if 'small-' in row[0]:
                dname = re.split('[-]',row[0])
                if int(dname[1])<11:
                    RMSETC.append([float(row[1]),abs(float(row[2])),"purple"])
                    L8+=1
                else:
                    RMSETC.append([float(row[1]),abs(float(row[2])),"pink"])
                    L9+=1
    ...
    print(f' small-05({L3}),small-099({L4}),smallupd-05({L8}),smallupd-099({L9}) &')
    print(f' TMP5LC7-099({L7}) are plotted')
    plt.close()