...
from RMSE/TC data of PCA-decompositted samples (dim=30,20,15,10 and no-PCA)
...
if __name__ == '__main__': 
    ...
    traindir=root+"result-pca/traincv/"
    plotdir=root+"result-pca/grpplot/"
    ...
    with open(rstfile, 'w') as rslt:
        ...
        for grp in grps:
            ...
            for j in range(1, 11):
                dataname=grp+"-"+str(j)
                datadir= root+"d20n50-PCA/"+grp+"/"+str(j)
                ...
                #Read 300K TC data and store the diff value from 112.1W/m-K                     
    ...
    fig = plt.figure()
    ax1 = fig.add_subplot(111)
    plt.title("Result of PCA decomposition (700samples)")
