...
if __name__ == '__main__': 
    ...
    dtn=0
    allplotfile=plotdir+"alldata2-bs.png"
    fig = plt.figure()
    ax3 = fig.add_subplot(111)
    plt.title("All data (w/o '--sym-fc' parameter: corrected)")
    ...
    for i, bsx in enumerate(bsxs):
        for j in range(0, 10):
            TCfile= root+"/"+bsx+"-d20n50/"+str(j+1)+"/predict-phono3py2/out.txt"
    ...
    for j in range(0, 10):
        TCfile= gdroot+"/d20n50/"+str(j+1)+"/predict-phono3py2/out.txt"
        with open(TCfile, 'r') as TCf:
            for n, line in enumerate(TCf):
                if 'Thermal conductivity (W/m-k)' in line:
                    TCf.seek(0)
