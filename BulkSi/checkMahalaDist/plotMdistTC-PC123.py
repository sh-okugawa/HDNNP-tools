...
"""
This script is calculating Mahalanobis distance of each predict's PC1&PC2&PC3
...
def plotL2TC(LC7root, outfolder, grps, md):
    ...
    plotfile=outfolder+"TCe&MD-PC123.png"
    ...
    for grpnum,grp in enumerate(grps):
        ...
        for i in range(1,10):
            ...
            for SFp in SFdtp123:
                ...
                Mdist123.append(MD)
            MDave=sum(Mdist123)/len(Mdist123)
        ...
        print(f'[{md}/{grp}] TC err & Ave of Mahalanobis distance of PC123 is plotted')
        
    plt.title(f'[{md}] TC err & Mean of Mahalanobis distance of PC1&PC2&PC3')
    ax.set_xlabel(f'TC Err (fm 112.1W/m-K:300K)')
    ax.set_ylabel(f'Mean of Mahalanobis dist (PC1&PC2&PC3)')
    ax.grid(True)
