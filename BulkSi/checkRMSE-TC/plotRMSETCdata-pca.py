...
from RMSE/TC data of PCA-dim-reduced samples (dim=10, 15, 20)
And plot train curve and scatter of force/RMSE&TC data
"""

if __name__ == '__main__': 
    pcafolder="/home/okugawa/HDNNP/Si-190808"
    grps=["p10","p15","p20"]

    os.makedirs(pcafolder+"/pca-result/traincv/plot")
    os.makedirs(pcafolder+"/pca-result/grpplot")
    rstfile=pcafolder+"/pca-result/RMSETCdata.csv"
    traindir=pcafolder+"/pca-result/traincv/"
    plotdir=pcafolder+"/pca-result/grpplot/"
    gdrstfile= pcafolder+"/result/RMSEdata.csv"
    ...
    with open(rstfile, 'w') as rslt:
        ...
        for grp in grps:
            ...
            for j in range(1, 11):
                dataname=grp+"-"+str(j)
                datadir= pcafolder+"/d20n50-"+grp+"/"+str(j)
    ...
    fig = plt.figure()
    ax1 = fig.add_subplot(111)
    plt.title("All data of PCA-reduced dim(10/15/20)")
    ax1.set_xlabel("TC Err (fm 112.1/300K)")
    ax1.set_ylabel("force/RMSE (meV/A)")
    ax1.grid(True)
    ...
    for j in range(0, 10):
        TCfile= pcafolder+"/d20n50/"+str(j+1)+"/predict-phono3py/out.txt"
        with open(TCfile, 'r') as TCf:
            for n, line in enumerate(TCf):
                if 'Thermal conductivity (W/m-k)' in line:
                    TCf.seek(0)
