...
def plotL2TC(LC7root, outfolder, ph3folder, grps, md):
    ...
    plotfileT=outfolder+"PCxmeanL2TTC.png"
    plotfileP=outfolder+"PCxmeanL2PTC.png"
    boxdic={"facecolor" : "lightcyan", "edgecolor" : "navy"}
    
    SFL2TTC, SFL2PTC= [],[]
    for grpnum,grp in enumerate(grps):
        clr=colors[grpnum]
        histfileT= outfolder+grp+"-PCxL2Thist.png"
        histfileP= outfolder+grp+"-PCxL2Phist.png"
        ...
        for i in range(1,11):
            ...
            if i==1:
                #Plot histgram of L2 norm based on PCx of Train symm_func
                ...
                xlbl="Euclid distance of Predict PCx"
                ttl="["+md+"/"+grp+"] Euclid distance of PCx (Train Symm_Func base)"
                plt.title(ttl)
                ax.set_xlabel(xlbl)
                ...
                #Plot histgram of L2 norm based on PCx of Predict symm_func
                ...
                ttl="["+md+"/"+grp+"] Euclid distance of PCx (Predict Symm_Func base)"
                plt.title(ttl)
                ax.set_xlabel(xlbl)
                ...
                print(f'[{md}/{grp}] Euclid distance of PCx is plotted')

    #Plot scatter of TC err & L2 norm based on Train symm_func            
    fig = plt.figure()
    ax = fig.add_subplot(111)
    plt.title(f'[{md}] TC err & Euclid distance of PCx (Train base)')
    ax.set_xlabel("TC Err (fm 112.1W/m-K:300K)")
    ax.set_ylabel("Mean of Predict PCx L2 norm")
    ax.grid(True)
    plt.rcParams["legend.edgecolor"] ='green'
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)
    plt.title(f'[{md}] TC err & Euclid distance of PCx (Predict base)')
    ax.set_xlabel("TC Err (fm 112.1W/m-K:300K)")
    ax.set_ylabel("Mean of Train PCx L2 norm")
    ax.grid(True)
    plt.rcParams["legend.edgecolor"] ='green'
...
if __name__ == '__main__': 
    ...
    outfolder=root+"result-LC7/PCx/L2-TC/"
    ...
    outfolder=root+"result-LC7n/PCx/L2-TC/"
    grps=['0.95','0.97','0.99','1.00','1.01','1.03','1.05','mix']
